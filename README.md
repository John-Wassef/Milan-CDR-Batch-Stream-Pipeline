...
![Pipeline Architecture](./Milan%20Project%20System%20Architecture.jpg)
...
* **Handle Missing/Invalid Values:** Fill null or negative values in numeric columns (`sms_in`, `sms_out`, `call_in`, `call_out`, `internet`) with `0.0`.  
* **Filter Invalid Records:** Retain only records where `cell_id` is between 1 and 10,000.  

### Aggregations
* **Daily:** Sum metrics grouped by `date` and `cell_id`.  
* **Hourly:** Sum metrics grouped by `date`, `hour`, and `cell_id` (streaming focuses on hourly).  

### ETL Flowchart
Below is a high-level diagram of the ETL pipelines:
